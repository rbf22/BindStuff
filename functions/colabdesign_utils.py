"""Utility functions for ColabDesign protein design workflows."""
####################################
...
def binder_hallucination(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches,too-many-statements,too-many-positional-arguments
    ...
    if advanced_settings["design_algorithm"] == "2stage":
        # uses gradient descend to get a PSSM profile and then uses PSSM to bias
        # the sampling of random mutations to decrease loss
        ...
        t_mcmc = 0.01
        af_model._design_mcmc(  # pylint: disable=protected-access
            advanced_settings["greedy_iterations"],
            half_life=half_life,
            T_init=t_mcmc,
        ...
        if initial_plddt > 0.65:
            ...
            if logits_iter > 0:
                ...
                af_model._tmp["seq_logits"] = af_model.aux["seq"]["logits"]  # pylint: disable=protected-access
            ...
            if softmax_plddt > 0.65:
                ...
                onehot_plddt = 0.0  # Initialize variable
    ...
    # let's check whether the trajectory is worth optimising by checking confidence,
    ...
    if advanced_settings["save_design_animations"]:
        ...
        with open(os.path.join(design_paths["Trajectory/Animation"],
                                      design_name+".html"), 'w', encoding='utf-8') as f:
            f.write(plots)
        plt.close('all')

    if advanced_settings["save_trajectory_pickle"]:
        with open(os.path.join(design_paths["Trajectory/Pickle"],
                                      design_name+".pickle"), 'wb') as handle:
...
def predict_binder_complex(  # pylint: disable=too-many-arguments,too-many-locals,too-many-positional-arguments,unused-argument
    ...
    for model_num in prediction_models:
        ...
        complex_pdb = os.path.join(design_paths["MPNN"],
        ...
        if not os.path.exists(complex_pdb):
            ...
            # perform initial AF2 values filtering to determine whether to skip relaxation
            # and interface scoring
            for filter_name, metric_key, comparison in filter_conditions:
                threshold = filters.get(filter_name, {}).get("threshold")
                if threshold is not None:
                    if (comparison == '>=' and
                        prediction_metrics[metric_key] < threshold):
    ...
    for model_num in prediction_models:
        complex_pdb = os.path.join(design_paths["MPNN"],
                                  f"{mpnn_design_name}_model{model_num+1}.pdb")
        if pass_af2_filters:
            mpnn_relaxed = os.path.join(design_paths["MPNN/Relaxed"],
                                       f"{mpnn_design_name}_model{model_num+1}.pdb")
...
def predict_binder_alone(  # pylint: disable=too-many-arguments,unused-argument,too-many-positional-arguments
    ...
    for model_num in prediction_models:
        # check to make sure prediction does not exist already
        binder_alone_pdb = os.path.join(design_paths["MPNN/Binder"],
                                       f"{mpnn_design_name}_model{model_num+1}.pdb")
...
def add_rg_loss(self: Any, weight: float = 0.1) -> None:
    ...
    self._callbacks["model"]["loss"].append(loss_fn)  # pylint: disable=protected-access
...
def add_i_ptm_loss(self: Any, weight: float = 0.1) -> None:
    ...
    self._callbacks["model"]["loss"].append(loss_iptm)  # pylint: disable=protected-access
...
def add_helix_loss(self: Any, weight: float = 0) -> None:
    ...
    def binder_helicity(inputs: Any, outputs: Any) -> Any:
        ...
        mask_2d = np.outer(  # pylint: disable=protected-access
            np.append(np.zeros(self._target_len), np.ones(self._binder_len)),  # pylint: disable=protected-access
            np.append(np.zeros(self._target_len), np.ones(self._binder_len)),  # pylint: disable=protected-access
        )
    ...
    self._callbacks["model"]["loss"].append(binder_helicity)  # pylint: disable=protected-access
